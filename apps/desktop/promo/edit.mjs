...
/**
 * The sound: one music bed under everything, and accents on the section starts.
 *
 * Generated by the project's own `stable_audio_3`, which fits in the container the video model does not —
 * eighty seconds of underscore in twelve seconds of compute. So the promo has sound even though it has
 * only one picture bed, which is the right way round: a silent promo reads as broken, a short one only
 * reads as short.
 *
 * The accent alternates riser and impact by section, with the tick cluster under the first: a transition
 * sound on every cut would be a parody of motion graphics, and the ear stops hearing an accent that never
 * varies.
 *
 * Levels are set here rather than left at unity. The bed sits under the accents by design — `-14 dB` in
 * linear terms — because the accents are the thing a viewer registers and the bed is what they feel.
 */
export const MUSIC = { asset: 'media/music.flac', gain: 0.2 };

export function accents({ frames = TOTAL_FRAMES } = {}) {
  const sections = Math.max(1, Math.floor(frames / SECTION_FRAMES));
  const made = [];

  for (let section = 0; section < sections; section += 1) {
    const at = section * SECTION_FRAMES;
    // A riser leads into an even section and an impact lands on an odd one, so consecutive sections do
    // not open the same way.
    made.push({
      asset: section % 2 === 0 ? 'media/riser.flac' : 'media/impact.flac',
      label: section % 2 === 0 ? 'riser' : 'impact',
      // The riser leads *into* the cut, so it starts before it; the impact lands *on* it.
      start: section % 2 === 0 ? Math.max(0, at - 20) : at,
      frames: section % 2 === 0 ? 60 : 40,
      gain: 0.7,
    });
  }

  // The ticks open the film, once, near the first title.
  made.push({ asset: 'media/ticks.flac', label: 'ticks', start: 12, frames: 70, gain: 0.5 });

  /*
   * Butt-jointed rather than layered, because they share a track.
   *
   * The editing rules refuse a collision instead of resolving it — a clip is never displaced to make room
   * — so a builder that emits two overlapping accents produces a document the application will not accept.
   * The opening riser and the tick cluster overlapped in the first version by exactly that much.
   *
   * Pushed to the previous end rather than dropped: an accent two frames late is still an accent, and
   * silently losing one would leave a cut with nothing on it for no stated reason.
   */
  const ordered = made.sort((left, right) => left.start - right.start);
  let clear = 0;
  for (const accent of ordered) {
    if (accent.start < clear) accent.start = clear;
    clear = accent.start + accent.frames;
  }
  return ordered;
}
