...
const checks = {
  ...
  'built-in uniforms are set from the layer': near(px('builtins')[0], 128) && near(px('builtins')[1], 255) && near(px('builtins')[2], 64),
  'broken shader degrades to passthrough':   near(px('brokenShader')[0], 255) && r?.brokenShader?.stats?.passesExecuted === 0,
  ...
  'missing texture skips only its layer':    near(px('missingTexture')[0], 255) && r?.missingTexture?.stats?.layersSkipped === 1,
